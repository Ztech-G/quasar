---
title: 配置 quasar.config 文件
desc: （@quasar/app - vite）在使用 Vite 的 Quasar CLI 应用中，你可以在哪些方面、如何进行配置以及配置的内容有哪些。
---

请注意，你搭建好的项目文件夹中包含一个 quasar.config 文件。那么你可以通过它配置哪些内容呢？基本上，Quasar CLI 为你做的所有事情都可以通过它来配置。

* 你将在你的网站/应用中使用的 Quasar 组件、指令和插件。
* 默认的 [Quasar 语言包](/options/quasar-language-packs)。
* 你希望使用的 [图标库](/options/installing-icon-libraries)。
* 用于 Quasar 组件的默认 [Quasar 图标集](/options/quasar-icon-sets)。
* 开发服务器端口、HTTPS 模式、主机名等等。
* 你希望使用的 [CSS 动画](/options/animations)。
* [启动文件](/quasar-cli-vite/boot-files)列表（这也决定了执行顺序）—— 这些是位于 `/src/boot` 目录下的文件，它们说明了在挂载根 Vue 组件之前你的应用是如何初始化的。
* 要包含在打包文件中的全局 CSS/Sass/…… 文件。
* 单页应用（SPA）、渐进式网络应用（PWA）、Electron 应用、Capacitor 应用、Cordova 应用、服务器端渲染（SSR）、浏览器扩展（BEX）的配置。
* 扩展底层工具，比如生成的 Vite 配置。
* …… 以及在这个过程中你还会发现的许许多多其他可配置项。

::: tip
你会注意到，更改这些设置中的任何一项都无需你手动重新启动开发服务器。Quasar 会检测到这些更改并重新加载必要的进程。你不会打断开发流程，因为你只需安心等待，Quasar CLI 会迅速重新加载更改后的代码，甚至还能保留当前状态。这能为你节省大量时间！
:::

::: warning
`/quasar.config` 文件由 Quasar CLI 构建系统执行，因此这段代码是直接在 Node 环境下运行的，而非在你的应用环境中运行。这意味着你可以引入像 `node:fs`、`node:path` 这样的 Node 内置模块以及 Vite 插件等等。
:::
...
你会注意到，`/quasar.config` 文件导出了一个函数，该函数接受一个 `ctx`（上下文）参数并返回一个对象。这使你能够根据这个上下文动态地更改你的网站/应用程序的配置：
...
这意味着，举个例子，当你针对某种特定模式（比如渐进式网络应用，即 PWA）进行构建时，你可以加载一种字体，而针对其他模式则选择另一种字体：
...
或者，你可以在单页应用（SPA）模式下使用一个全局 CSS 文件，而在Cordova模式下使用另一个全局 CSS 文件，同时避免在其他模式下加载任何此类文件。
...
或者，你可以将开发服务器配置为在单页应用（SPA）模式下运行在 8000 端口，在渐进式 Web 应用（PWA）模式下运行在 9000 端口，或者在其他模式下运行在 9090 端口。
...
注意从 `#q-app/wrappers` 导入的 `defineConfig`。它本质上是一个无实际操作的函数，但它的作用是提升集成开发环境（IDE）的自动补全体验。
...
更多信息请查看[预取功能](/quasar-cli-vite/prefetch-feature)页面。
...
除了这些选项之外，Quasar CLI 还会对一些设置进行处理，并且你在使用时会发现与普通 Vite 应用有所不同：

可以使用 `open` 属性指定用特定的浏览器打开应用，而不是使用操作系统的默认浏览器（查看[支持的值](https://github.com/sindresorhus/open#options)）。前面链接中描述的 `options` 参数就是你应该在 `quasar.config` 文件的 `devServer > open` 里进行配置的内容。以下是一些示例：
...
你可以在`/index.html`中定义变量，然后引用它们，就像这样：
...
请参考我们文档中[添加到 process.env](/quasar-cli-vite/handling-process-env#adding-to-process-env) 这一部分的内容。
